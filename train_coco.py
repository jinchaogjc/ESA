...
from core.active.build import PixelSelection, RegionSelection, SuperPixelSelection, EntitySelection, EntitySegSelection, SuperPixelRIPUSelection, SuperPixelActiveSelection
...
from tools.utils import init_spixel_grid, build_LABXY_feat, label2one_hot_torch
from core.loss.superpixel_loss import compute_semantic_pos_loss
...
def train(cfg):
    ...
    for batch_index, (src_data, tgt_data) in enumerate(zip(src_train_loader, tgt_train_loader)):
        ...
        if cfg.ACTIVE.SETTING == 'SPARIW':
            weight = torch.ones(cfg.MODEL.NUM_CLASSES) * 0.9
            # leave background = 0, out, from VOC to COCO id.
            trainid2name_keys = [0, 5, 2, 15, 9, 40, 6, 3, 16, 57, 20, 61, 17, 18, 4, 1, 59, 19, 58, 7, 63]
            weight[trainid2name_keys] = torch.tensor([1.1, 1.1, 2.0, 1.1, 1.1, 2.0, 1.1, 1.1, 1.1, 3.0, 1.1, 2.0, 1.1, 1.1, 1.1, 1.1, 2.0, 1.1, 2.0, 1.1, 1.1], dtype=torch.float)
            sup_criterion = nn.CrossEntropyLoss(ignore_index=255, weight=weight.cuda())
        ...
        # if cfg.ACTIVE.SETTING == 'FS' and iteration == 0:
        #     # cp target_mask to correspond area.
        #     pass
        ...
        # superpixel loss
        if cfg.SOLVER.SUPERPIXEL_LOSS > 0:
            DOWNSIZE=16
            POS_WEIGHT=0.003
            spixel_id, xy_feat = init_spixel_grid(train_img_height=cfg.INPUT.TARGET_INPUT_SIZE_TRAIN[1], train_img_width=cfg.INPUT.TARGET_INPUT_SIZE_TRAIN[0], 
                                                       input_img_height=cfg.INPUT.SOURCE_INPUT_SIZE_TRAIN[1], input_img_width=cfg.INPUT.SOURCE_INPUT_SIZE_TRAIN[0], 
                                                       downsize=DOWNSIZE, batch_size=cfg.SOLVER.BATCH_SIZE, b_train=True)
            label_1hot = label2one_hot_torch(tgt_mask.to(device), C=cfg.MODEL.NUM_CLASSES) # set C=50 as SSN does
            LABXY_feat_tensor = build_LABXY_feat(label_1hot, xy_feat) 
            superpixel_loss , loss_sem, loss_pos = compute_semantic_pos_loss(predict, LABXY_feat_tensor,
                                                   pos_weight=POS_WEIGHT, kernel_size=DOWNSIZE, target_mask=tgt_mask)
            superpixel_loss = superpixel_loss * cfg.SOLVER.SUPERPIXEL_LOSS
            meters.update(sp_loss=superpixel_loss.item())
            loss += superpixel_loss
        ...
        # active learning
        if iteration in cfg.ACTIVE.SELECT_ITER or (cfg.DEBUG and iteration == 20):
            if cfg.ACTIVE.SETTING == "RA":
                ...
                PixelSelection(cfg=cfg,
                               ...
                               classifier=classifier,
                               tgt_epoch_loader=tgt_epoch_loader)
            elif cfg.ACTIVE.SETTING == 'SPA':
                SuperPixelSelection(cfg=cfg,
                               tgt_epoch_loader=tgt_epoch_loader)
            elif cfg.ACTIVE.SETTING == 'ESA':
                EntitySelection(cfg=cfg,
                               tgt_epoch_loader=tgt_epoch_loader)
            elif cfg.ACTIVE.SETTING == 'ESSA':
                EntitySegSelection(cfg=cfg,
                               tgt_epoch_loader=tgt_epoch_loader)
            elif cfg.ACTIVE.SETTING == 'PARIPU' or cfg.ACTIVE.SETTING == 'SPARIPU':
                SuperPixelRIPUSelection(cfg=cfg,
                               feature_extractor=feature_extractor,
                               classifier=classifier,
                               tgt_epoch_loader=tgt_epoch_loader)
            elif cfg.ACTIVE.SETTING == 'SPALL' or cfg.ACTIVE.SETTING == 'SPARIPU' \
                or cfg.ACTIVE.SETTING == 'SPARANDOM' or cfg.ACTIVE.SETTING == 'SPAPU' \
                or cfg.ACTIVE.SETTING == 'SPARI' or cfg.ACTIVE.SETTING == 'SPARIW':
                SuperPixelActiveSelection(cfg=cfg,
                               feature_extractor=feature_extractor,
                               classifier=classifier,
                               tgt_epoch_loader=tgt_epoch_loader)
            active_round += 1
