...
def get_files_ids_from_txt(txt_path):
    # fo = open(txt_path, "r")
    imgs = []
    with open(txt_path) as f:
        for line in f.readlines():
            imgs.append(int(line.strip().split('.')[0]))
    return imgs
...
def generate_coco_train_list(debug):
    
    input = ["datasets/coco/images/train2017/*.jpg"]
    # input = glob.glob(os.path.expanduser(input[0]))
    input = glob.glob(input[0])
    # print(input)
